 ...
 * ships against the canonical `cuba-banks` collection and fails on any drift,
 ...
 *   CANONICAL_FILE=../../my-ussd-codes/my-ussd-codes-collection/codes/cuba-banks.json \
 *     node .github/scripts/check-ussd-sync.mjs
 */

import fs from "node:fs";

const CANONICAL_ID = "cuba-banks";
const CANONICAL_URL = `https://raw.githubusercontent.com/albertolicea00/MyUSSDCodes-collection/main/codes/${CANONICAL_ID}.json`;
const LOCAL_FILE = "BancaRemota/codes.json";

/** Repo-specific: this catalog nests banks -> categories -> operations. */
function extractLocal(json) {
  const out = [];
  for (const bank of json.banks ?? []) {
    for (const cat of bank.categories ?? []) {
      for (const op of cat.operations ?? []) {
        if (op.ussdCode) out.push(op.ussdCode);
      }
    }
  }
  return out;
